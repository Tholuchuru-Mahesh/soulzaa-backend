...
export class PostScoreListener implements OnModuleInit {
  ...
  onModuleInit(): void {
    ...
    this.bus.subscribe<PostUnlikedEvent>(POST_EVENTS.UNLIKED, (e) =>
      this.bump(e.payload.postId, -1, 0),
    );
    this.bus.subscribe<PostCommentedEvent>(POST_EVENTS.COMMENTED, (e) =>
      this.bump(e.payload.postId, 0, 1),
    );
    this.bus.subscribe<PostCommentDeletedEvent>(POST_EVENTS.COMMENT_DELETED, (e) =>
      this.bump(e.payload.postId, 0, -1),
    );
  }
